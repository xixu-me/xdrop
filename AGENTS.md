...
Install JS dependencies once with `npm ci`.

- `npm run dev:web`: start the Vite dev server for the frontend.
- `npm run build:web`: type-check and build the web app, then generate SEO assets.
- `npm run lint:web`: run ESLint on `apps/web`.
- `npm run test:web` / `npm run test:web:coverage`: run Vitest, with or without coverage output.
- `npm run test:e2e`: run the Playwright suite in `tests/e2e`.
- `npm run format` / `npm run format:check`: run Prettier plus Go formatting checks.
