...
describe('UploadStudio helpers', () => {
  ...
  it('reads dropped file entries and nested directory entries', async () => {
    ...
    const directoryEntry: EntryLike = {
      ...
      name: 'photos',
      createReader: () => {
        const batches = [[nestedEntry], [], []] satisfies EntryLike[][]
        return {
          readEntries: (callback: (entries: EntryLike[]) => void) =>
            callback(batches.shift() ?? []),
        }
      },
    }
    const multiBatchDirectoryEntry: EntryLike = {
      isDirectory: true,
      isFile: false,
      name: 'archive',
      createReader: () => {
          ...
          [
            {
              ...
              name: 'first.txt',
              file: (callback: (value: File) => void) =>
                callback(new File(['first'], 'first.txt', { lastModified: 4 })),
            },
          ],
          [
            {
              ...
              name: 'second.txt',
              file: (callback: (value: File) => void) =>
                callback(new File(['second'], 'second.txt', { lastModified: 5 })),
            },
          ],
          [],
        ] satisfies EntryLike[][]
    ...
    await expect(__test__.readEntry(multiBatchDirectoryEntry)).resolves.toEqual([
      {
        file: expect.any(File),
        relativePath: 'archive/first.txt',
      },
      {
        file: expect.any(File),
        relativePath: 'archive/second.txt',
      },
    ])
    await expect(
