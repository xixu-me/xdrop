...
func TestManageTransferEndpointsRejectInvalidPayloadsAndMissingTokens(t *testing.T) {
	...
	patchUnauthorized := performAuthorizedJSONRequest(
		t,
		router,
		nethttp.MethodPatch,
		"/api/v1/transfers/"+createResponse.TransferID+"/",
	...
	require.Equal(t, nethttp.StatusUnauthorized, patchUnauthorized.Code)
	require.Contains(t, patchUnauthorized.Body.String(), `"error":"missing_manage_token"`)
}
